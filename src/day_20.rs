...
pub fn archive_files_size(data: Vec<u8>) -> Result<Json<u64>, (Status, String)> {
    tar::Archive::new(Cursor::new(data))
        .entries()
...
pub async fn cookie(data: Vec<u8>) -> Result<String, (Status, String)> {
    ...
    tar::Archive::new(Cursor::new(data))
        ...
        .and_then(|_| {
            ...
            for (author, commit) in String::from_utf8(output.stdout)
                .map_err(map_err::<500>)?
                .lines()
...
fn find_santa(pat: PathBuf) -> Result<PathBuf, ()> {
    ...
    if pat.is_file()
        && pat
            .file_name()
            .expect("Dear god why does this not have a name")
            == OsStr::new("santa.txt")
    {
        return Ok(pat);
    }
